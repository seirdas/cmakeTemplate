...
// Componentes: Controles generales -----------------------------------------------------
     ...
     * @return @c true Si se ha mandado el comando, false en caso contrario
     */
    bool set_Mute(unsigned int id, bool mute);


// Componentes: Dynamics ----------------------------------------------------------------

    // #TODO
    bool set_MonoGate_Threshold(int id, double dB);


// Componentes: Routers & Selectors -----------------------------------------------------

    // #TODO
    bool set_InputSelector_Selection(int id, unsigned int sel);
...
// Búsqueda de parámetros ---------------------------------------------------------------
    ...
    /**
     * @brief Busca la posición de un parámetro en la caché local mediante su identificador.
     * * @param id Identificador único del parámetro de Symetrix a buscar.
     * @return int El índice base cero en la caché si se encuentra; de lo contrario, -1.
     */
     ...
     */
    int findOrAlloc(int id, Type t, int defaultOffsetTicks);
...
/************ Variables ********************************************************/
    ...
    // Estado    
    bool        initialized_;         ///< Bandera de inicialización interna de la instancia de la clase.
    ...
    // Offsets por defecto (no se pueden cambiar en caliente)
    struct Offsets {
        double voxDb = 1.0;          // solo puertas de ruido
        double gainDb = 0.3;         // faders/ganancias
        int    selectorTicks = 1;    // selectores 2 estados
        int    superMatrixCenti = 30;// 0.30 dB por cruce (centi-dB)
    };
    Offsets m_offsets_{}; // siempre los valores por defecto

    
    // --- Conexión de socket ---
    std::chrono::steady_clock::time_point m_lastPingTime_;  ///< Registro temporal del último comando de control o ping enviado al hardware.
    ...
    static constexpr int    kMaxEntries  = 500;   ///< Capacidad máxima del array de caché para elementos de control independientes.
    Entry m_cache_[kMaxEntries]{};                ///< Array estático que funciona como caché local para evitar saturar el bus UDP con valores idénticos.
    int   m_count_{ 0 };                          ///< Número actual de entradas activas y registradas en el array m_cache.
    ...
    std::vector<int> m_smPrevCenti;               ///< Buffer histórico de ganancias de los cruces de la matriz guardado en centi-dB para control de cambios.
    std::vector<unsigned short> m_smOffsetCenti;  ///< Tolerancia de cambio guardada por celda expresada en centi-dB (extraída de m_offsets).
    ...
    Offsets m_offsets{};                          ///< Instancia que contiene las tolerancias de variación por defecto para cada tipo de dato.
    unsigned int const kBootPreset = 1;           ///< Número de preset de hardware (1..1000) que se invocará automáticamente al arrancar. Si es 0 se omite.
};