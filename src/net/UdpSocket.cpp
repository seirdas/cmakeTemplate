...
void UdpSocket::start() {
    ...
    if (!initialized_) {
        SYS_WARN("UdpSocket","Socket " + name_ + ":" + std::to_string(local_endpoint_.port()) + " not initialized.");
        return;
    }
...
bool UdpSocket::openSocket() {
    ...
    if (ec) {
        if (ec == asio::error::access_denied)
            ...
            SYS_WARN("UdpSocket","Port " + std::to_string(local_endpoint_.port()) + " already in use.");
        else
